    ...
    <article className="min-w-[260px] max-w-[280px] shrink-0 overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm transition-shadow hover:shadow-md">
      <div className="relative h-32 bg-zinc-100">
        <img
          ...
          decoding="async"
        />
        <span className="absolute left-2 top-2 rounded-md bg-rva-gold px-2 py-0.5 text-[10px] font-bold text-rva-slate">
          RVA pick
        </span>
      </div>
      <div className="p-3">
        ...
        <p className="truncate text-xs font-semibold text-rva-river">{event.venue}</p>
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="text-xs font-bold text-rva-river hover:underline"
          >
            Map
          </button>
        </div>
        <a
          ...
          className="mt-2 flex w-full items-center justify-center rounded-md bg-rva-river py-2 text-xs font-bold text-white hover:bg-rva-river-light"
        >
          Get tickets
        </a>
      </div>
...
export default function FeaturedEventsStrip({ events, onSelectEvent }) {
  ...
  return (
    <section className="mb-10" aria-labelledby="featured-strip-heading">
      <h2 id="featured-strip-heading" className="font-display text-xl font-bold text-rva-slate">
        On the bill around town
      </h2>
      <p className="mt-1 text-sm text-rva-slate/75">
        Highlights from the current RVA feed — swaps as CultureWorks and partners refresh their calendars.
      </p>
